...
struct HiddenTravelRouterTests {
    ...
    private func makeFillObject(name: String, color: UInt32 = 0x000000) -> (object: EmbroideryObject, points: [Point2D]) {
    ...
    private func makeLineObject(name: String, from a: Point2D, to b: Point2D, color: UInt32 = 0x000000) -> (object: EmbroideryObject, points: [Point2D]) {
    ...
    @Test func bridgesGapWhenPathIsCoveredByNextShape() {
        ...
        #expect(bridged[1].points.count > b.points.count, "bridging should prepend extra stitch points")
        #expect(bridged[1].points.last == b.points.last, "B's own generated points must still end exactly where they did")
        let prependedCount = bridged[1].points.count - b.points.count
        for p in bridged[1].points.prefix(prependedCount) {
    ...
    @Test func doesNotBridgeWhenPathLeavesShape() {
        ...
        let bridged = HiddenTravelRouter.bridgeSameColorGaps([a, b], thresholdMM: 5.0)
        #expect(bridged[1].points == b.points)
    }
