#!/usr/bin/env python3
"""
Independent EXP validation oracle used only by StitchPilotCoreTests
(see TESTING.md). Parses an EXP file with pyembroidery (a separate
...
(see EXPFormat.swift's "Coordinate convention" note) -- this script negates
...
def main() -> None:
    if len(sys.argv) != 2:
        print("usage: validate_exp.py <path.exp>", file=sys.stderr)
        sys.exit(2)
