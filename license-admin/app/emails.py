...
def send_system(key: str, *, to_email: str, customer_id: Optional[int] = None, vars: Optional[dict] = None, reply_to: str = "", attachments: Optional[list] = None, footer_note: str = "",
    ...
    `from_email` overrides the sender for the same reason one level up: a
    stream of its own is not enough while the From domain is shared."""
    ...
    msg = email_sender._compose(to_email=to_email, subject=subject, body=body, html_body=html, reply_to=reply_to,
                                unsubscribe_url=unsubscribe_url, from_email=from_email)
...
def run_scheduled_work() -> dict:
    ...
    result = {"sent": 0, "winback": 0, "proofs": 0, "lapsed": 0, "recurring": 0, "backfilled": 0, "outreach": 0, "announcements": 0, "backup": 0}
    try:
    ...
    try:
        from . import backups
        result["backup"] = backups.check()
    except Exception as e:  # noqa: BLE001
        log.exception("Backup check failed: %s", e)
    try:
